...
inline bool is_quantized(const QuantizationParams& params) { return params.num_channels() > 0; }
inline bool is_per_channel_quantization(const QuantizationParams& params) { return params.num_channels() > 1; }
...
class PerChannelQuantizationParams : public QuantizationParams {
  public:
    //PerChannelQuantizationParams(const int32_t* zp, const int32_t* scale, int num_channels) : _zp(zp), _scale(scale), _num_channels(num_channels) {}

    //Smarter compile time thing might be 
    template<int num_channels>
    PerChannelQuantizationParams(const int32_t(& zp)[num_channels], const int32_t(& scale)[num_channels]) : QuantizationParams(zp, scale, num_channels) {}
