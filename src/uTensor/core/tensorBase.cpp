...
TensorBase::TensorBase(){
    Context::get_default_context().register(*this);
}
...
const IntegralValue TensorInterface::operator()(uint16_t i, uint16_t j, uint16_t k, uint16_t l) const {
    // Add shape checks here
    return read(_shape.linear_index(i, j, k, l));
}
IntegralValue TensorInterface::operator()(uint16_t i, uint16_t j, uint16_t k, uint16_t l){
    // Add shape checks here
    return write(_shape.linear_index(i,j,k,l));
}
size_t TensorInterface::get_readable_block(void* buffer, uint16_t req_read_size,  uint32_t linear_index) const {
    printf("ERROR, Optimized op attempted to read access non-optimizable tensor\n");
    return -1;
}
size_t TensorInterface::get_writeable_block(void* buffer,uint16_t req_write_size, uint32_t linear_index){
