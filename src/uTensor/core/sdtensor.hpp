...
//                         offset + ele < cursor)) {
//      // 1. dirty && miss state
//      mem.flush_data<T>(_filename, unit_size(), s->cache_size, s->total_size,
//                        cursor, (T*)s->data);
//      mem.load_data<T>(_filename, unit_size(), s->cache_size, s->total_size,
//                       offset, (T*)s->data);
//      cursor = offset;
//      dirty = false;
//    } else if (!dirty && (offset + ele > cursor + s->cache_size ||
...
//  }
//  void initCache() {
//    mem.load_data<T>(_filename, unit_size(), s->cache_size, s->total_size, 0,
//                     (T*)s->data);
//  }
//
...
//
//  // virtual void* read(size_t offset, size_t ele) override{};
//  virtual uint16_t unit_size(void) override { return sizeof(T); }
//
//  ~SDTensor() {}
//
// private:
