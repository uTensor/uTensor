...
S_TENSOR Context::add(Tensor* t, TName _name, uint8_t init_count) {
  if (t == nullptr) {
    ERR_EXIT("null pointer tensor");
  }
  if (rTable.find(_name) != rTable.end()) {
    ERR_EXIT("tensor with name \"%d\" address already exist in rTable",
             t->getName().get_value());
  }
...
S_TENSOR Context::get(TName const& t_name) {
  if (rTable.find(t_name) == rTable.end())
    ERR_EXIT("No tensor with name: %d", t_name.get_value());
  return rTable[t_name].ptr;
}

Operator* Context::registerOpTable(std::function<void*(void)> func,
                                   TName _name) {
  Operator* op;
...
void Context::push(Operator* op, TName _name, TNameList _inputs,
                   TNameList _outputs) {
  Context::push(op, _inputs, _outputs);
}

void Context::push(Operator* op, std::initializer_list<TName> _inputs,
...
void Context::push(Operator* op, TNameList& in_names, TNameList& out_names) {
  ...
  for (auto in : in_names) {
    if (rTable.find(in) == rTable.end()) {
      ERR_EXIT("Tensor \"%d\" not found", in.get_value());
    }
  ...
  for (auto out : out_names) {
    if (rTable.find(out) == rTable.end()) {
      ERR_EXIT("Tensor \"%d\" not found", out.get_value());
    }
