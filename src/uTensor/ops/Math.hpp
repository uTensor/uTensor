#ifndef UTENSOR_MATH_OPS_H
#define UTENSOR_MATH_OPS_H
#include "context.hpp"
...
void arg_max_kernel(Tensor& output, Tensor& input, Tensor& axis, CompareFlag cmp = Max) {
    if (axis->get_type() != u32) {
        uTensor_printf("only support u32 type axis tensor\n");
    ...
    uint16_t axis_size = input_shape[0];
    int num_dims = input_shape.num_dims();
    if (axis_dim < 0) {
        axis_dim += input_shape.num_dims();
    }
