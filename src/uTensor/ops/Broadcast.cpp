...
void
Broadcaster::set_shape( const TensorShape& shape_a, const TensorShape& shape_b ){
...
void
Broadcaster::set_shape( const TensorShape& shape_a, const TensorShape& shape_b, const TensorShape& shape_c ) {
    uint8_t num_dims_a = shape_a.num_dims(), num_dims_b = shape_b.num_dims(), num_dims_c = shape_c.num_dims();
    _strides_c = TensorStrides(_shape_c);
    uint32_t s_a = 1, s_b = 1;
    for (uint8_t offset = 0; offset < num_dims_c; offset++) {
        int c_idx = num_dims_c - offset - 1, a_idx = num_dims_a - offset - 1, b_idx = num_dims_b - offset - 1;
        if ( a_idx < 0 ) {
            _strides_a[c_idx] = 0;
        }
...
std::pair<uint32_t, uint32_t> Broadcaster::get_linear_idx( uint32_t idx_c ) const {
...
bool is_broadcastable( const TensorShape& shape_a, const TensorShape& shape_b, TensorShape& shape_c ) {
