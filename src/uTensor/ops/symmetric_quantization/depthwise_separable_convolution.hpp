...
public:
  ...
  QuantizedDepthwiseSeparableConvOperator(std::initializer_list<uint16_t> strides, Padding padding, const int depth_multiplier=1, const uint16_t (&dialation)[2]={1,1});
  
  // activation basically only used for TESTING, USE AT YOUR OWN RISK
  QuantizedDepthwiseSeparableConvOperator(std::initializer_list<uint16_t> strides, Padding padding, const int depth_multiplier=1, const uint16_t (&dialation)[2]={1,1}, const TFLM::TfLiteFusedActivation activation=TFLM::kTfLiteActNone);

  static void calculateOpData(const Tensor& input, const Tensor& filter, const Tensor& bias, Tensor& output, 
      const uint16_t (&strides)[4], const Padding padding, 
      const int depth_multiplier,
      const uint16_t (&dialations)[2],
      int output_shift,
      ...
      int32_t& output_activation_min,
      int32_t& output_activation_max,
      TFLM::TfLiteFusedActivation=TFLM::kTfLiteActNone // Make this param basically not required
      );
...
template <typename Tout>
QuantizedDepthwiseSeparableConvOperator<Tout>::QuantizedDepthwiseSeparableConvOperator() : _stride{1,1}, _padding(SAME), depth_multiplier(1), _dialation{1,1}, output_multiplier(1), output_shift(0), per_channel_output_multiplier(nullptr), per_channel_output_shift(nullptr), output_activation_min(std::numeric_limits<Tout>::min()), output_activation_max(std::numeric_limits<Tout>::max()) {} 

template <typename Tout>
QuantizedDepthwiseSeparableConvOperator<Tout>::QuantizedDepthwiseSeparableConvOperator(std::initializer_list<uint16_t> strides, Padding padding, const int depth_multiplier, const uint16_t (&dialation)[2], TFLM::TfLiteFusedActivation activation)
      : _padding(padding), depth_multiplier(depth_multiplier), _dialation{dialation[0], dialation[1]}, activation(activation) {
    int i = 0;
    for (auto s : strides) {
      _stride[i++] = s;
    }
...
void QuantizedDepthwiseSeparableConvOperator<Tout>::calculateOpData(
    ...
    TFLM::ComputePaddingHeightWidth(
        stride_height, stride_width, 1, 1, height, width,
        filter_height, filter_width, &padding_height, &padding_width, static_cast<TFLM::TfLitePadding>(padding), &unused_output_height,
        &unused_output_width);
    
    int num_channels =
        filter->get_shape()[TFLM::kDepthwiseConvQuantizedDimension];
    QuantizationParams affine_quantization =
        filter->get_quantization_params();
    ...
    for (int i = 0; i < num_channels; ++i) {
      ...
      const float scale = is_per_channel ? filter->get_quantization_params()
...
void QuantizedDepthwiseSeparableConvOperator<Tout>::compute() {
  AllocatorInterface* ram_allocator = Context::get_default_context()->get_ram_data_allocator();
    ...
    int num_channels =
     ...
     per_channel_output_multiplier = reinterpret_cast<int32_t*>(ram_allocator->allocate(sizeof(int32_t)*num_channels));
     per_channel_output_shift = reinterpret_cast<int32_t*>(ram_allocator->allocate(sizeof(int32_t)*num_channels));
    ...
    calculateOpData(inputs[in].tensor(), inputs[filter].tensor(), inputs[bias].tensor(), outputs[out].tensor(), 
        _stride, _padding, depth_multiplier, _dialation, output_shift, 
        per_channel_output_multiplier,
        per_channel_output_shift, 
        paddingVals.width, paddingVals.height,
        output_multiplier,
