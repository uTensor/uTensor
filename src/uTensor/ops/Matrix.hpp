...
void matrix_mult_kernel(Tensor& c, const Tensor& a, const Tensor& b){
    ...
    if(a_shape.num_dims() > 2 || b_shape.num_dims() > 2 || c_shape.num_dims() > 2 || a_shape[1] != b_shape[0] || a_shape[0] != c_shape[0] || b_shape[1] != c_shape[1]){
      printf("[Error] Invalid matrix multiple shape mismatch\n");
      Context::get_default_context()->throwError(new InvalidMatrixMultIndicesError);
    }
    ...
    for(uint32_t i = 0; i < a_shape[0]; i++){
      for(uint32_t j = 0; j < b_shape[1]; j++) {
         ...
         for(uint32_t k = 0; k < a_shape[1]; k++){
            ...
            //printf("i, j, k : %d %d %d %d %d\n", i, j, k, static_cast<T>(a(i, k)) , static_cast<T>(b(k, j)));
         }
         c(i, j) = tmp;
      }
    }
...
protected:
    virtual void compute() {
        matrix_mult_kernel<T>(*outputs[c].tensor, *inputs[a].tensor, *inputs[b].tensor);
    }
};
