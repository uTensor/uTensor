...
class StridedSliceOperator : public OperatorInterface<9, 1> {
 ...
 protected:
  virtual void compute() {
    stridedslice_kernel<T>(inputs[i].tensor(), inputs[b].tensor(),inputs[e].tensor(), inputs[s].tensor(), outputs[o].tensor(), inputs[b_m].tensor(), inputs[ell_m].tensor(), inputs[e_m].tensor(), inputs[n_a_m].tensor(), inputs[s_a_m].tensor());
  }
};
}
}
