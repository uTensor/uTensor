#ifndef UTENSOR_LEGACY_QUANTIZATION_UTILS
#define UTENSOR_LEGACY_QUANTIZATION_UTILS
#include <math.h>
#include <limits>
#include <cstdlib>
...
inline void RequantizeManyInNewRange(const Tensor& input, uint32_t count,
  ...
  for (size_t index = 0; index < count; ++index) {
    const float input_float =
        QuantizedToFloat<T1>(static_cast<T1>(input(index)), min_input, max_input);
...
void RequantizeManyInNewRangeReference(const Tensor& /*int*/ input, int32_t count,
    float min_input, float max_input,
    float min_output,
    float max_output,
