...
void QuantizedAdd<uint8_t, uint8_t, int>(
  ...
  const float x_min_float = min_x;
  const float x_max_float = max_x;
  const float y_min_float = min_y;
  const float y_max_float = max_y;
  ...
  out_min = output_min_float;
  out_max = output_max_float;
  ...
  uint32_t input_elems_cnt = input_x->get_shape().num_elems();
  uint32_t smaller_elems_cnt = input_y->get_shape().num_elems();
  ...
  for (size_t off = 0; off < input_elems_cnt; ++off) {
    ...
    int32_t x_value_32 = static_cast<int32_t>( static_cast<uint8_t>(input_x(off)));
    ...
    int32_t y_value_32 = static_cast<int32_t>(static_cast<uint8_t>(input_y(idx)));
