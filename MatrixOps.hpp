...
void ReferenceGemmuImpl(bool transpose_a, bool transpose_b, bool transpose_c,
                        ...
                        size_t lda, const T2* b, int offset_b, size_t ldb, T3* c,
                        int shift_c, int offset_c, int mult_c, size_t ldc)
{
...
template <class T>
int64_t FloatToQuantizedUnclamped(float input, float range_min, float range_max)
{
    const int64_t lowest_quantized = static_cast<double>(std::numeric_limits<T>::lowest());
    ...
    const double range_scale = (number_of_steps / range);  //NT: fractional resolution?  bit per floating-point-increment
    int64_t quantized =
        (round(input * range_scale) - round(range_min * range_scale));  //NT: roughly: (input - range_min) * range_scale
    quantized += lowest_quantized;  //NT: can be a negative number for <signed ...>; zero gets down-shifted to Eigen::NumTraits<T>::lowest()
    return quantized;  //NT: input<float> qanuntized to a specific range<int64>
}

template <class T>
float FloatForOneQuantizedLevel(float range_min, float range_max)    //NT: information loss if float_for_one_quantized_level < 1
{
                                        ...
                                        float* max_c)
{
    ...
    const int64_t c_highest = static_cast<int64_t>(std::numeric_limits<T3>::max());
    const int64_t c_lowest = static_cast<int64_t>(std::numeric_limits<T3>::lowest());
    const float c_float_for_one_quant_level =
        a_float_for_one_quant_level * b_float_for_one_quant_level;

    *min_c = c_float_for_one_quant_level * c_lowest;  //NT: this resulting in taking only the necessary quantize range
    *max_c = c_float_for_one_quant_level * c_highest;
}
template <class T1, class T2, class Toutput>
void QuantizedMatMul(Tensor<T1> A, Tensor<T2> B, Tensor<Toutput> C,
                     Tensor<float> mina, Tensor<float> minb, Tensor<float> maxa, Tensor<float> maxb,
                     Tensor<float> outmin, Tensor<float> outmax)
{
    ...
    const int32_t offset_a = FloatToQuantizedUnclamped<T1>(0.0f, min_a, max_a);  //NT: what 0 quantized to; depends on Eigen::NumTraits<T>::lowest()
    ...
    ReferenceGemmuImpl<T1, T2, Toutput>(transpose_a, transpose_b, transpose_c,
                                        m, n, k, A_Data, offset_a,
                                        lda, B_Data, offset_b, ldb, C_Data,
                                        shift_c, offset_c, mult_c, ldc);
    float min_c_value;
    float max_c_value;

    QuantizationRangeForMultiplication<T1, T2, Toutput>(min_a, max_a,
            min_b, max_b, &min_c_value, &max_c_value);
...
public:
    void qMatMul(void)
    {
        ...
        Tensor<float> b_min = t_import.float_import("/fs/testData/qMatMul/in/qB_1.idx");
        Tensor<float> b_max = t_import.float_import("/fs/testData/qMatMul/in/qB_2.idx");
        ...
        //Sub-functions: QuantizationRangeForMultiplication, QuantizationRangeForMultiplication, FloatForOneQuantizedLevel

        Tensor<int> out_c(c.getShape());
        Tensor<float> out_min(c_min.getShape());
        Tensor<float> out_max(c_max.getShape());
        QuantizedMatMul<uint8_t, uint8_t, int>(a, b, out_c, a_min,
                                               b_min, a_max, b_max,
                                               out_min, out_max);
        //
        ...
        double result = meanPercentErr(c, out_c) + meanPercentErr(c_min, out_min) + meanPercentErr(c_max, out_max);
