...
output_file = "test_maxpool.cpp"
const_file = "constants_maxpool.hpp"
...
TEST(MaxPool, random_inputs_{{ test_name }}) {
  ...
  MaxPoolOp<float> mxpool({ {% for x in ksize %}{{ x }}{{ "," if not loop.last }}{% endfor %}}, { {% for x in strides %}{{ x }}{{ "," if not loop.last }}{% endfor %}}, {{ PADDING }});
  mxpool
