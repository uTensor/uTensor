...
class QuantizedMatMulOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    const int32 offset_a = FloatToQuantizedUnclamped<T1>(0.0f, min_a, max_a);  //NT: what 0 quantized to; depends on Eigen::NumTraits<T>::lowest()
    ...
    const size_t m = a.dim_size(a_dim_remaining);  //NT: Convenience accessor for the tensor shape.
