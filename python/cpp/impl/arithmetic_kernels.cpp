...
py::array_t<T> add_kernel(const py::array_t<T> &a, const py::array_t<T> &b) {
  ...
  for (int i = 0; i < info_a.ndim; i++)
    shape_a[i] = info_a.shape[i];
  for (int i = 0; i < info_b.ndim; i++)
    shape_b[i] = info_b.shape[i];
  shape_a.update_dims();
  shape_b.update_dims();
  ...
  Tensor tensor_a = new RamTensor(shape_a, elem_type);
  Tensor tensor_b = new RamTensor(shape_b, elem_type);
  Tensor tensor_c = new RamTensor(output_shape, elem_type);
  copy_op.toTensor(info_a.ptr, tensor_a);
  copy_op.toTensor(info_b.ptr, tensor_b);
  uTensor::add_kernel<T>(tensor_c, tensor_a, tensor_b);
...
template py::array_t<float> add_kernel(const py::array_t<float> &a,
                                       const py::array_t<float> &b);
template py::array_t<int8_t> add_kernel(const py::array_t<int8_t> &a,
                                        const py::array_t<int8_t> &b);
template py::array_t<uint8_t> add_kernel(const py::array_t<uint8_t> &a,
                                         const py::array_t<uint8_t> &b);
template py::array_t<int16_t> add_kernel(const py::array_t<int16_t> &a,
                                         const py::array_t<int16_t> &b);
template py::array_t<uint16_t> add_kernel(const py::array_t<uint16_t> &a,
                                          const py::array_t<uint16_t> &b);
template py::array_t<int32_t> add_kernel(const py::array_t<int32_t> &a,
                                         const py::array_t<int32_t> &b);
template py::array_t<uint32_t> add_kernel(const py::array_t<uint32_t> &a,
                                          const py::array_t<uint32_t> &b);
