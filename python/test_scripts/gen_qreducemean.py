...
test_group = "ReduceMean"
num_tests = 4
test_base = "reduceMean"
output_file = "test_sq_%s.cpp" % test_base
const_file = "constants_sq_%s.hpp" % test_base
operator_name = "ReduceMeanOperator"
operator_var = "reduceMeanOp"

def gen_test(test_number):
    test_name = "random_gen_%s__%d" % ( test_base, test_number)
    in0 = np.random.uniform(low=-20, high=50, size=(4,5,6,7)).astype(np.float32)
    axis = 2
    out_1 = tf.reduce_mean(in0, axis=axis).numpy()
    ...
    param_str = "{%d}" % axis
    op = Operator(operator_name, operator_var, dtypes=[in_t.get_dtype], param_str=param_str)
