...
test_name = "1_dws_conv"
output_file = "test_quantized_dws_conv.cpp"
const_file = "constants_quantized_dws_conv.hpp"
...
TEST(Quantized, reference_{{ test_name }}) {
  ...
  Tensor {{ tensor.name }} =  new RomTensor({ {% for x in tensor.shape %}{{ x }}{{ ", " if not loop.last }}{% endfor %} }, {{ tensor.utype }}, {{ tensor.r_name }});
                        {{ tensor.name }}->set_quantization_params(PerTensorQuantizationParams({{ tensor.r_zp_name }}[0], {{ tensor.r_scale_name }}[0]));

  {% endfor %}
  Tensor out = new RamTensor({ {% for x in out_tensor.shape %}{{ x }}{{ ", " if not loop.last }}{% endfor %} }, {{ out_tensor.utype }});
                out->set_quantization_params(PerTensorQuantizationParams({{ out_tensor.r_zp_name }}[0], {{ out_tensor.r_scale_name }}[0] ));
